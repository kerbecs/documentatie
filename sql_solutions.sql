...
FROM employees
         JOIN jobs USING (job_id)
WHERE salary > ANY (SELECT avg_salary
...
WHERE salary > (SELECT * FROM avg_salary)
  AND department_id IN (SELECT * FROM it_departments);
...
WITH avg_salary("average_salary") AS (SELECT AVG(salary) FROM employees),
     ...
     laura_department_id("dep_id") AS (SELECT department_id
                                       FROM departments
...
SELECT first_name, last_name, COUNT(employee_id) AS "employees_supervised"
FROM employees
GROUP BY(manager_id)
